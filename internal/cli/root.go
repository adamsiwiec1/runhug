...
func printUsage(w io.Writer) {
	...
	fmt.Fprintln(w, "  wizard             guided setup (guide, guided, setup)")
	...
	fmt.Fprintln(w, "  list               local registry + runpod (deployments)")
	fmt.Fprintln(w, "  proxy              openai proxy :8080/v1 (serve)")
	fmt.Fprintln(w, "  gpus / import / delete / use / url / status")
	fmt.Fprintln(w)
	fmt.Fprintln(w, bold("chat & agents"))
	fmt.Fprintln(w, "  run [model]        chat repl (-q one-shot)")
	fmt.Fprintln(w, "  start <agent>      coding agent bridge (claude)")
	fmt.Fprintln(w)
	fmt.Fprintln(w, bold("local"))
	fmt.Fprintln(w, "  local add          models on this machine")
	fmt.Fprintln(w, "  local setup        ollama / llama.cpp / mlx")
	fmt.Fprintln(w)
