...
func (h *HF) Embed(ctx context.Context, query string, docs []string) ([]float32, [][]float32, error) {
	if h == nil || strings.TrimSpace(h.Token) == "" {
		return nil, nil, fmt.Errorf("hf embed: HF_TOKEN required")
	}
	texts := make([]string, 0, 1+len(docs))
	texts = append(texts, clipEmbed(query))
	for _, d := range docs {
		texts = append(texts, clipEmbed(d))
	}
	vecs, err := h.embedAll(ctx, texts)
	if err != nil {
		return nil, nil, err
	}
	if len(vecs) != len(texts) {
		return nil, nil, fmt.Errorf("hf embed: got %d vectors for %d texts", len(vecs), len(texts))
	}
...
func parseFeatureExtraction(raw []byte, n int) ([][]float32, error) {
	...
	// Single vector for a single input.
	if n == 1 && len(vecs) > 0 && len(vecs[0]) > 0 && len(vecs) != n {
		// already sentence vectors but wrong count — if one nested, handled above
	}
	if n == 1 && len(vecs) >= 1 {
	...
	case []any:
		...
		if inner, ok := x[0].([]any); ok {
			if len(inner) > 0 {
				...
				// token-level: mean pool each document
				out := make([][]float32, 0, len(x))
				for _, doc := range x {
					tokens, ok := doc.([]any)
					if !ok {
