...
func PrintConfig(w io.Writer, s Snapshot) {
	...
	fmt.Fprintln(w, "  runpod-vllm-proxy init")
	fmt.Fprintln(w, "  runpod-vllm-proxy init --model qwen3:8b")
	fmt.Fprintln(w)
	fmt.Fprintln(w, "search Hugging Face")
	fmt.Fprintln(w, "  runpod-vllm-proxy search qwen --sort likes")
	fmt.Fprintln(w, "  runpod-vllm-proxy inspect Qwen/Qwen2.5-7B-Instruct")
	fmt.Fprintln(w, "  runpod-vllm-proxy local add --pick 1")
	fmt.Fprintln(w)
	fmt.Fprintln(w, "Runpod")
	fmt.Fprintln(w, "  runpod-vllm-proxy connect")
	fmt.Fprintln(w, "  runpod-vllm-proxy deploy Qwen/Qwen2.5-7B-Instruct")
	fmt.Fprintln(w, "  runpod-vllm-proxy list")
	fmt.Fprintln(w, "  runpod-vllm-proxy proxy")
	fmt.Fprintln(w)
	fmt.Fprintln(w, "already running a server")
	fmt.Fprintln(w, "  runpod-vllm-proxy local add --name ollama --url http://127.0.0.1:11434/v1")
	fmt.Fprintln(w, "  runpod-vllm-proxy local add --name mlx    --url http://127.0.0.1:8082/v1")
	fmt.Fprintln(w)
